...
export default {
  ...
  methods:{
      ...
      save(){
          var that=this;
          let id=JSON.parse(sessionStorage.indexLogin).uid
          let title=this.title;
          let con=this.con;
          fetch('/ajax/publish/addCon',{
              method:'post',
              headers:{
              ...
              that.Conboxs=!that.Conboxs;
              that.ctitle=''
              that.con=''
              that.listData.unshift(e[0]);
          })
      },
      num(){
    ...
    created(){
      var that=this;
        fetch('/ajax/list/select').then(function (e) {
            return e.json()
        }).then(function (e) {
            that.listData=e;
        })
